...
/* Stable public payment/order API. This Tailscale Funnel hostname persists
   across restarts, unlike the former trycloudflare quick-tunnel addresses. */
var PP_API='https://bennies-mac-mini.tail121a00.ts.net:8443';

/* Wix can keep the old document alive during client-side route changes. The
   storefront is route-initialized, so force one real document load whenever
   Wix changes the pathname underneath the running bundle. */
;(function(){try{
  var loadedPath=(location.pathname.replace(/\/+$/,'')||'/');
  var reloading=false;
  function recoverRoute(){
    var currentPath=(location.pathname.replace(/\/+$/,'')||'/');
    if(reloading||currentPath===loadedPath)return;
    reloading=true;
    location.reload();
  }
  addEventListener('popstate',recoverRoute);
  setInterval(recoverRoute,250);
}catch(e){}})();
...
;(function(){try{
  ...
  var REVIEW_ENDPOINT='https://shakespeare-somerset-moments-mazda.trycloudflare.com/track';
...
(function(){
  ...
  var metas=[
    ...
    ['og:image','https://static.wixstatic.com/media/e76a5f_398beccf075b4f0c93d26b2a6ae2c997~mv2.png/v1/fill/w_1200,h_630,al_c,q_85,enc_auto/e76a5f_398beccf075b4f0c93d26b2a6ae2c997~mv2.png'],
    ['og:image:width','1200'],
    ['og:image:height','630'],
    ['og:locale','en_US'],
...
/* ============================================================
   ...
   ============================================================ */
(function(){
  // Search and social crawlers must see the public page content without an
  // interaction-only overlay. Human visitors still receive age verification.
  var crawlerUA=/bot|crawler|spider|slurp|bingpreview|facebookexternalhit|twitterbot|linkedinbot|whatsapp|google-inspectiontool|lighthouse/i;
  if(crawlerUA.test(navigator.userAgent||''))return;
  ...
  function showGate(){
    ...
    document.getElementById('pp-av-enter').onclick=function(){
      ...
      try{var x=new XMLHttpRequest();x.open('POST','https://shakespeare-somerset-moments-mazda.trycloudflare.com/track',true);x.setRequestHeader('Content-Type','application/json');x.send(JSON.stringify({vid:(localStorage.getItem('pp_vid')||''),event:'lead_capture',detail:lead,ts:Date.now(),page:location.pathname}));}catch(e){}
...
/* ============================================================
   ...
   ============================================================ */
(function(){
  var ANALYTICS_URL='https://shakespeare-somerset-moments-mazda.trycloudflare.com';
...
function injectSEOSchema(){
  ...
  addLD({
    ...
    "sameAs":[
      ...
      "https://www.tiktok.com/@precisionusalabs",
      "https://www.linkedin.com/company/precisionusalabs/"
    ],
    "contactPoint":{
      "@type":"ContactPoint",
      "telephone":"+17253396023",
  ...
  if(products.length>0){
    ...
    for(var i=0;i<products.length;i++){
      var pr=products[i];
      var imgUrl=pr[1][0]==='h'?pr[1]:B+pr[1]+'/v1/fit/w_600,h_600,q_85,enc_auto/'+pr[1];
...
["GHKCU Copper Serum 1 oz","e76a5f_5c74bfdbc317480e9dd07e6ef0ea0168~mv2.png","ghkcu-copper-serum",29.99],
...
["Retatrutide 10mg","e76a5f_125227f9b07c4f18976a60ed7f8cd28b~mv2.png","retatrutide-10mg",200],
["Retatrutide 20mg","https://cdn.jsdelivr.net/gh/vegasblackcard/pp-pay@3fd0432/img/reta-20mg.png","retatrutide-20mg",260],
...
var perfCss=document.createElement('style');perfCss.textContent='.pp-hero{aspect-ratio:3/2}.pp-hero img{width:100%!important;height:100%!important;object-fit:cover!important}.pp-card-img img{aspect-ratio:1/1}';document.head.appendChild(perfCss);
...
function rca(list){var h='';list.forEach(function(p){var remote=p[1][0]==='h',im=remote?p[1]:B+p[1]+'/v1/fit/w_320,h_320,q_72,enc_auto/'+p[1],im2=remote?'':B+p[1]+'/v1/fit/w_180,h_180,q_68,enc_auto/'+p[1];h+='<div data-slug="'+p[2]+'" class="pp-card" style="cursor:pointer"><div class="pp-card-img"><img src="'+im+'"'+(remote?'':' srcset="'+im2+' 180w, '+im+' 320w" sizes="(max-width:768px) 44vw, 260px"')+' alt="'+p[0]+'" loading="lazy" decoding="async" width="320" height="320"></div><div class="pp-card-info"><h3>'+p[0]+'</h3><div class="pp-price">$'+p[3]+'</div><span class="pp-atc">Add to Cart</span></div></div>';});return h;}
...
var bsH=rc('BS');d.innerHTML='<div class="pp-hero"><img src="https://cdn.jsdelivr.net/gh/vegasblackcard/pp-pay@e028441/assets/hero/precision-hero-720.webp" srcset="https://cdn.jsdelivr.net/gh/vegasblackcard/pp-pay@e028441/assets/hero/precision-hero-480.webp 480w, https://cdn.jsdelivr.net/gh/vegasblackcard/pp-pay@e028441/assets/hero/precision-hero-720.webp 720w, https://cdn.jsdelivr.net/gh/vegasblackcard/pp-pay@e028441/assets/hero/precision-hero-1080.webp 1080w" sizes="100vw" alt="Precision Labs research products" fetchpriority="high" decoding="async" width="1080" height="720"></div>'
...
+'<h1 style="font-family:Archivo,sans-serif;font-size:32px;font-weight:800;color:#0e1b4d;margin:0 0 16px;line-height:1.2">Precision Labs Research Products</h1>'
...
+'<li style="font-size:13px;color:#000;padding:6px 0;border-bottom:1px solid #e5e7eb">\u2713 Research documentation support</li>'
+'<li style="font-size:13px;color:#000;padding:6px 0;border-bottom:1px solid #e5e7eb">\u2713 Research starter kit</li>'
+'<li style="font-size:13px;color:#000;padding:6px 0;border-bottom:1px solid #e5e7eb">\u2713 Laboratory handling supplies</li>'
+'<li style="font-size:13px;color:#000;padding:6px 0;border-bottom:1px solid #e5e7eb">\u2713 Batch documentation access</li>'
...
/* ============================================================
   FEATURED PRODUCT — direct crawlable link to a priority page
   ============================================================ */
+'<div style="max-width:980px;margin:0 auto 26px;padding:0 20px"><div style="background:#fff;border:1px solid #dde2f0;border-radius:16px;padding:22px 20px;box-shadow:0 4px 18px rgba(14,27,77,.06)"><p style="margin:0 0 8px;font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:#4770db;font-weight:700">Featured Research Product</p><h3 style="font-family:Archivo,sans-serif;font-size:24px;color:#0e1b4d;margin:0 0 8px">Retatrutide 10mg documentation hub</h3><p style="margin:0 0 14px;font-size:14px;color:#444;line-height:1.7">Review the product page, batch documentation, and lab-testing resources for one of the site\'s highest-interest research products. Research use only. Not for human consumption.</p><div style="display:flex;gap:12px;flex-wrap:wrap"><a href="/product-page/retatrutide-10mg" style="display:inline-block;padding:12px 20px;border-radius:40px;background:#4770db;color:#fff;font-weight:700;text-decoration:none">View Product</a><a href="/lab-testing" style="display:inline-block;padding:12px 20px;border-radius:40px;background:#0e1b4d;color:#fff;font-weight:700;text-decoration:none">See Lab Testing</a><a href="/post/how-to-read-a-peptide-coa-in-2026-hplc-mass-spec-and-lot-matching" style="display:inline-block;padding:12px 20px;border-radius:40px;background:#eef2fd;color:#0e1b4d;font-weight:700;text-decoration:none">Read COA Guide</a></div></div></div>'
...
'ghkcu-copper-serum':'GHKCU Copper Serum is a topical GHK-Cu copper-peptide preparation studied in research on skin-related pathways such as collagen and elasticity. Investigated in laboratory research for antioxidant activity. 1 fl oz / 30 ml bottle. Research purposes only — not for human consumption.',
...
window.ppAffiliate={codes:{'FIRST10':{discount:0.10,label:'FIRST ORDER - 10% OFF'},'KIM10':{discount:0.10,label:'KIM - 10% OFF'},'MAGGIE':{discount:0.10,label:'MAGGIE - 10% OFF'},'GEORGE':{discount:0.07,label:'GEORGE - 7% OFF'},'JORGE':{discount:0.07,label:'JORGE - 7% OFF'},'KATHRYN':{discount:0.07,label:'KATHRYN - 7% OFF'},'RICKYJ':{discount:0.15,label:'RICKY J - 15% OFF'},'RICKY J':{discount:0.15,label:'RICKY J - 15% OFF'},'JESS':{discount:0.15,label:'JESS - 15% OFF'},'JESS15':{discount:0.15,label:'JESS - 15% OFF'},'JADE':{discount:0.15,label:'JADE - 15% OFF'},'ALEX':{discount:0.15,label:'ALEX - 15% OFF'},'CHERA88':{discount:0.12,label:'12% OFF'},'CODE12':{discount:0.12,label:'12% OFF'},'12':{discount:0.12,label:'12% OFF'}},applied:null,getDiscount:function(subtotal){if(!this.applied)return 0;var code=this.codes[this.applied];return code?Math.round(subtotal*code.discount*100)/100:0;},apply:function(code){var upper=code.toUpperCase().trim();if(this.codes[upper]){this.applied=upper;try{sessionStorage.setItem('pp_applied',upper);}catch(e){}return{success:true,label:this.codes[upper].label};}return{success:false};},remove:function(){this.applied=null;try{sessionStorage.removeItem('pp_applied');localStorage.removeItem('pp_applied');}catch(e){}},addCode:function(code,discount,label){this.codes[code.toUpperCase()]={discount:discount,label:label};try{var stored=JSON.parse(localStorage.getItem('pp-codes')||'{}');stored[code.toUpperCase()]={discount:discount,label:label};localStorage.setItem('pp-codes',JSON.stringify(stored));}catch(e){}}};try{var saved=JSON.parse(localStorage.getItem('pp-codes')||'{}');for(var k in saved)window.ppAffiliate.codes[k]=saved[k];}catch(e){}try{localStorage.removeItem('pp_applied');var _sa=sessionStorage.getItem('pp_applied');if(_sa&&window.ppAffiliate.codes[_sa]){window.ppAffiliate.applied=_sa;}else{sessionStorage.removeItem('pp_applied');window.ppAffiliate.applied=null;}}catch(e){}
...
window.ppReconCalc=function(){
var ex=document.getElementById('pp-rc-overlay');if(ex){ex.remove();document.documentElement.classList.remove('pp-rc-open');document.body.classList.remove('pp-rc-open');return;}
if(!document.getElementById('pp-rc-css')){var s=document.createElement('style');s.id='pp-rc-css';s.textContent='html.pp-rc-open,body.pp-rc-open{overflow:hidden!important;height:100%!important}body.pp-rc-open>*:not(#pp-rc-overlay):not(#pp-age-gate){display:none!important;visibility:hidden!important}#pp-rc-overlay{position:fixed;inset:0;z-index:2147483000;background:#0e1b4d!important;overflow-y:auto;-webkit-overflow-scrolling:touch;font-family:Questrial,-apple-system,BlinkMacSystemFont,sans-serif}#pp-rc-overlay *{box-sizing:border-box}.pp-rc-in{max-width:560px;margin:0 auto;padding:24px 20px 60px}.pp-rc-x{position:fixed;top:14px;right:16px;z-index:99995;background:rgba(255,255,255,.12);color:#fff;border:none;border-radius:50%;width:40px;height:40px;font-size:24px;cursor:pointer}.pp-rc-eyebrow{color:#d4af37;font-family:Archivo,sans-serif;font-size:13px;font-weight:700;letter-spacing:3px;margin:8px 0 6px}.pp-rc-h1{font-family:Archivo,sans-serif;font-size:34px;font-weight:800;color:#fff;line-height:1.05;margin:0 0 10px;letter-spacing:-.5px}.pp-rc-sub{color:#aab4c8;font-size:15px;line-height:1.5;margin:0 0 24px}.pp-rc-lbl{color:#fff;font-family:Archivo,sans-serif;font-size:16px;font-weight:700;margin:22px 0 10px}.pp-rc-row{display:flex;flex-wrap:wrap;gap:10px}.pp-rc-pill{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);color:#dfe5f2;padding:12px 20px;border-radius:30px;font-size:15px;cursor:pointer;transition:.15s}.pp-rc-pill.on{background:#4770db!important;border-color:transparent;color:#fff;font-weight:700;box-shadow:0 4px 16px rgba(71,112,219,.4)}.pp-rc-inp{width:100%;padding:14px;border:none;border-radius:10px;font-size:16px;margin:10px 0 0;background:#f4f6fb;color:#0e1b4d;font-weight:600}.pp-rc-out{background:#16224a!important;border:1px solid rgba(212,175,55,.35);border-radius:16px;padding:22px 20px;margin:26px 0 0}.pp-rc-out-lbl{color:#aab4c8;font-size:14px;margin:0 0 4px}.pp-rc-out-big{font-family:Archivo,sans-serif;font-size:44px;font-weight:800;color:#fff;line-height:1;margin:0 0 6px}.pp-rc-out-big span{color:#d4af37;font-size:24px;font-weight:700}.pp-rc-out-sub{color:#8fa0c0;font-size:14px;margin:6px 0 0}.pp-rc-warn{color:#ffb4a0;font-size:13px;margin:10px 0 0}.pp-rc-disc{color:#7a86a6;font-size:12px;line-height:1.6;margin:24px 0 0;padding:16px 0 0;border-top:1px solid rgba(255,255,255,.1)}';document.head.appendChild(s);}
var st={syr:1.0,vial:10,bac:2,target:250,unit:'mcg'};
function grp(label,key,opts,other){var h='<div class="pp-rc-lbl">'+label+'</div><div class="pp-rc-row" data-g="'+key+'">';for(var i=0;i<opts.length;i++){h+='<div class="pp-rc-pill" data-v="'+opts[i][0]+'">'+opts[i][1]+'</div>';}if(other)h+='<div class="pp-rc-pill" data-v="other">Other</div>';h+='</div>';if(other)h+='<input class="pp-rc-inp" id="pp-rc-'+key+'-in" type="number" inputmode="decimal" placeholder="Enter '+label.toLowerCase()+'" style="display:none">';return h;}
var ov=document.createElement('div');ov.id='pp-rc-overlay';
ov.innerHTML='<button class="pp-rc-x" id="pp-rc-x">&times;</button><div class="pp-rc-in"><div class="pp-rc-eyebrow">REFERENCE TOOL</div><h1 class="pp-rc-h1">Reconstitution<br>Reference</h1><p class="pp-rc-sub">Concentration &amp; reconstitution reference. Enter your own values — this tool does not recommend or suggest any dose.</p>'+grp('Syringe size','syr',[[0.3,'0.3 mL'],[0.5,'0.5 mL'],[1.0,'1.0 mL'],[3.0,'3.0 mL']],true)+grp('Vial quantity (mg)','vial',[[5,'5 mg'],[10,'10 mg'],[15,'15 mg'],[20,'20 mg']],true)+grp('Bacteriostatic water added','bac',[[1,'1 mL'],[2,'2 mL'],[3,'3 mL'],[5,'5 mL']],true)+grp('Target amount','target',[[50,'50'],[100,'100'],[250,'250'],[500,'500'],[1000,'1000']],true)+'<div class="pp-rc-row" data-g="unit" style="margin-top:10px"><div class="pp-rc-pill" data-v="mcg">mcg</div><div class="pp-rc-pill" data-v="mg">mg</div></div><div class="pp-rc-out"><div class="pp-rc-out-lbl">Draw to</div><div class="pp-rc-out-big" id="pp-rc-units">—</div><div class="pp-rc-out-sub" id="pp-rc-detail"></div><div class="pp-rc-warn" id="pp-rc-warn"></div><div id="pp-rc-syr" style="margin-top:20px;overflow-x:auto"></div></div><div class="pp-rc-disc">Reference / educational tool for laboratory research only. Not medical, dosing, or health advice, and not a suggestion of any dose. Products are not for human or animal consumption. Consult a qualified professional. Calculations assume a U-100 insulin syringe (100 units = 1 mL).</div></div>';
document.body.appendChild(ov);document.documentElement.classList.add('pp-rc-open');document.body.classList.add('pp-rc-open');
function paint(){['syr','vial','bac','target','unit'].forEach(function(k){var row=ov.querySelector('[data-g="'+k+'"]');if(!row)return;var v=String(st[k]);var found=false;row.querySelectorAll('.pp-rc-pill').forEach(function(p){var on=(p.dataset.v===v);p.classList.toggle('on',on);if(on)found=true;});var oin=document.getElementById('pp-rc-'+k+'-in');if(oin){if(!found){var op=row.querySelector('[data-v="other"]');if(op)op.classList.add('on');oin.style.display='block';}else{oin.style.display='none';}}});}
function drawSyr(units,maxU,ml){var W=340,x0=18,x1=322,bw=x1-x0,by=46,bh=42;var majorStep=maxU<=30?5:(maxU<=60?10:(maxU<=100?10:50));var minorStep=majorStep/5;var ticks='',labels='';for(var t=0;t<=maxU+0.0001;t+=minorStep){var x=x0+(t/maxU)*bw;var maj=Math.abs(t/majorStep-Math.round(t/majorStep))<0.01;var th=maj?bh*0.72:bh*0.4;ticks+='<line x1="'+x.toFixed(1)+'" y1="'+(by+bh-6)+'" x2="'+x.toFixed(1)+'" y2="'+(by+bh-6-th).toFixed(1)+'" stroke="'+(maj?'#cdd7ee':'#7f8db0')+'" stroke-width="'+(maj?1.6:0.9)+'"/>';}for(var m=majorStep;m<=maxU+0.0001;m+=majorStep){var lx=x0+(m/maxU)*bw;labels+='<text x="'+lx.toFixed(1)+'" y="30" fill="#aab4c8" font-size="12" font-family="Archivo,sans-serif" text-anchor="middle">'+Math.round(m)+'</text>';}var frac=Math.max(0,Math.min(1,units/maxU));var fx=x0+frac*bw;var fill='<rect x="'+x0+'" y="'+by+'" width="'+(frac*bw).toFixed(1)+'" height="'+bh+'" rx="4" fill="#4770db" opacity="0.5"/>';var marker=(units>0&&frac<=1)?'<line x1="'+fx.toFixed(1)+'" y1="'+(by-4)+'" x2="'+fx.toFixed(1)+'" y2="'+(by+bh+4)+'" stroke="#d4af37" stroke-width="2"/>':'';var head='<div style="display:flex;justify-content:space-between;align-items:center;margin:0 0 8px"><span style="color:#fff;font-family:Archivo,sans-serif;font-weight:700;font-size:14px">Syringe Visual</span><span style="color:#aab4c8;font-size:13px">'+ml+' mL / '+maxU+' units</span></div>';return head+'<svg viewBox="0 0 '+W+' 100" style="width:100%;height:auto;display:block"><rect x="'+(x0-6)+'" y="'+(by-6)+'" width="'+(bw+22)+'" height="'+(bh+12)+'" rx="12" fill="rgba(255,255,255,.05)" stroke="rgba(255,255,255,.15)"/>'+fill+ticks+marker+'<rect x="'+(x1+6)+'" y="'+(by+bh/2-1.5)+'" width="14" height="3" rx="1.5" fill="#cdd7ee"/>'+labels+'</svg>';}
function compute(){var conc=st.vial/st.bac;var doseMg=(st.unit==='mcg')?st.target/1000:st.target;var vol=doseMg/conc;var units=vol*100;var maxU=st.syr*100;var u=Math.round(units*10)/10;document.getElementById('pp-rc-units').innerHTML=(isFinite(u)&&u>0?u:'—')+' <span>units</span>';document.getElementById('pp-rc-detail').textContent=(isFinite(vol)&&vol>0)?('= '+(Math.round(vol*1000)/1000)+' mL  ·  concentration '+(Math.round(conc*100)/100)+' mg/mL'):'';document.getElementById('pp-rc-warn').textContent=(isFinite(units)&&units>maxU)?('⚠ Exceeds '+st.syr+' mL syringe capacity ('+maxU+' units) — use a larger syringe or more BAC water.'):'';var syr=document.getElementById('pp-rc-syr');if(syr)syr.innerHTML=drawSyr(units,maxU,st.syr);}
ov.addEventListener('click',function(e){var p=e.target.closest('.pp-rc-pill');if(!p)return;var row=p.closest('[data-g]');var k=row.dataset.g;var v=p.dataset.v;if(v==='other'){var oin=document.getElementById('pp-rc-'+k+'-in');if(oin){oin.style.display='block';oin.focus();}row.querySelectorAll('.pp-rc-pill').forEach(function(x){x.classList.remove('on');});p.classList.add('on');return;}st[k]=(k==='unit')?v:parseFloat(v);paint();compute();});
['syr','vial','bac','target'].forEach(function(k){var oin=document.getElementById('pp-rc-'+k+'-in');if(oin)oin.addEventListener('input',function(){var val=parseFloat(oin.value);if(!isNaN(val)&&val>0){st[k]=val;compute();}});});
document.getElementById('pp-rc-x').onclick=function(){ov.remove();document.documentElement.classList.remove('pp-rc-open');document.body.classList.remove('pp-rc-open');};
paint();compute();
};
...
try{var _pc=(window.ppAffiliate&&ppAffiliate.applied)||'';if(_pc){try{var _em=(document.getElementById('pp-ship-email')||{}).value||'';navigator.sendBeacon(PP_API+'/api/promo-track',JSON.stringify({code:_pc,method:method,amount:amt,email:_em}));}catch(e){}}}catch(e){}
try{
...
try{var orderData={orderId:oid,vid:(localStorage.getItem('pp_vid')||''),customerName:name,customerEmail:email,customerPhone:phone,shippingAddress:fullAddr,paymentMethod:method,items:orderItems,subtotal:subtotal,shippingMethod:shipMethod,shippingFee:shipFee,orderTotal:amt,total:amt,estimatedDelivery:shipDelivery,orderDate:ts};
...
/* Funnel attribution: only Square posts a purchase (on its ?order=confirmed redirect), so
   Zelle/Venmo/Cash App/crypto orders never reached the tracker and the funnel read
   "checkouts started, 0 completed". Record those here as pending_manual at confirm time. */
try{if(!/^Square/.test(String(method))){var _mo={};for(var _k in orderData){_mo[_k]=orderData[_k];}_mo.source='manual_pending';_mo.status='pending_manual';
fetch('https://shakespeare-somerset-moments-mazda.trycloudflare.com/api/purchase',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(_mo)}).catch(function(){});
fetch(PP_API+'/api/record-purchase',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(_mo)}).catch(function(){});}}catch(_me){}
return orderData;
...
if(enabled){try{fetch(PP_API+'/api/email-capture',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({brand:'pp',source:'abandoned_checkout',email:document.getElementById('pp-ship-email').value,meta:{amount:amt,phone:document.getElementById('pp-ship-phone').value,firstOrderCode:'FIRST10',trust:'COA available on request'}})}).catch(function(){});}catch(e){}var note='Order: ';var cart=ppCart.get();for(var s in cart){var p=ppCart.find(s);if(p)note+=p[0]+' x'+cart[s]+', ';}note+='Ship to: '+document.getElementById('pp-ship-name').value+', '+document.getElementById('pp-ship-addr').value+(document.getElementById('pp-ship-apt').value?' '+document.getElementById('pp-ship-apt').value:'')+', '+document.getElementById('pp-ship-city').value+' '+document.getElementById('pp-ship-state').value+' '+document.getElementById('pp-ship-zip').value+' | Ph: '+document.getElementById('pp-ship-phone').value+' | Email: '+document.getElementById('pp-ship-email').value;
...
window.ppSquareCheckout=function(amt){var emailRaw=(document.getElementById('pp-ship-email')||{}).value||'';var email=/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailRaw)?emailRaw:'';var pending=ppNotifyOrder('Square (Card/Apple Pay)',amt)||{};try{localStorage.setItem('pp_pending_square_order',JSON.stringify(pending));}catch(e){}var b=document.getElementById('pp-square-btn');if(b){b.textContent='Loading secure checkout…';b.style.pointerEvents='none';}var _sqUrl=PP_API+'/api/square-checkout';var _sqBody=JSON.stringify({amountCents:Math.round(amt*100),email:email,redirectUrl:'https://www.precisionusalabs.com/?order=confirmed&pp_order='+encodeURIComponent(pending.orderId||'')});var _sqFail=function(){alert('Card checkout temporarily unavailable \u2014 use Zelle below (5% off).');if(b){b.textContent='Pay by Card / Apple Pay \u2014 $'+amt.toFixed(2);b.style.pointerEvents='auto';}};/* The payment service uses a stable public route, but brief network errors can still occur; one blip used to bounce the customer straight to "unavailable". Retry twice before giving up. */var _sqTry=function(n){fetch(_sqUrl,{method:'POST',headers:{'Content-Type':'application/json'},body:_sqBody}).then(function(r){return r.json();}).then(function(d){if(d&&d.url){window.location.href=d.url;}else if(n>0){setTimeout(function(){_sqTry(n-1);},1200);}else{_sqFail();}}).catch(function(){if(n>0){setTimeout(function(){_sqTry(n-1);},1200);}else{_sqFail();}});};_sqTry(2);};
...
var h='';res.forEach(function(p){var im=p[1][0]==='h'?p[1]:B+p[1]+'/v1/fit/w_400,h_400,q_85,enc_auto/'+p[1];h+='<div data-slug="'+p[2]+'" style="display:flex;align-items:center;gap:10px;padding:10px 14px;cursor:pointer;border-bottom:1px solid #f0f0f0"><img src="'+im+'" style="width:40px;height:40px;object-fit:contain"><div style="flex:1"><div style="font-weight:600;color:#0e1b4d;font-size:14px">'+p[0]+'</div><div style="color:#555;font-size:13px">$'+p[3]+'</div></div></div>';});
...
document.getElementById('pp-cart-btn').onclick=function(){window.location.href='/cart-page'+((window.ppAffiliate&&ppAffiliate.applied)?('?promo='+ppAffiliate.applied):'');};
mn.onclick=function(e){var a=e.target.closest('[data-nav]');if(!a)return;mn.classList.remove('open');mo.classList.remove('open');var n=a.dataset.nav;if(n==='lab-testing'){if(window.ppShowLabTesting)window.ppShowLabTesting();return;}if(n==='calc'){if(window.ppReconCalc)window.ppReconCalc();return;}var U={home:'/',store:'/',about:'/about-us',services:'/services',blog:'/blog',contact:'/contact',faq:'/faq',terms:'/terms',disclaimer:'/disclaimer',research:'/research-use'};if(U[n])window.location.href=U[n];};
}
}
...
ab.innerHTML='<h1>About Us</h1><p class="pp-badge">VETERAN OWNED</p><p>Precision Labs is a veteran-owned research supplier providing research-grade peptide formulations and clear batch documentation.</p><h2 style="text-align:center">Our Mission</h2><p>Provide consistent research materials, transparent testing documentation, and responsive support for qualified research customers.</p><h2 style="text-align:center">Why Choose Us</h2><p>Every product is sourced from compliant facilities and independently tested for purity. Certificates of Analysis are available on request.</p><p style="margin-top:24px"><span onclick="window.location.href=\'/contact\'" style="display:block;width:100%;padding:14px;background:#4770db;color:#fff;border-radius:40px;font-size:16px;font-weight:600;text-align:center;cursor:pointer">Contact Us</span></p>';
...
document.getElementById('pp-book-form').onsubmit=function(e){e.preventDefault();var form=this,btn=form.querySelector('button[type="submit"]'),nm=document.getElementById('pp-bk-name').value.trim(),em=document.getElementById('pp-bk-email').value.trim(),ph=document.getElementById('pp-bk-phone').value.trim(),tp=document.getElementById('pp-bk-type').value,nt=(document.getElementById('pp-bk-notes').value||'').trim();btn.disabled=true;btn.textContent='Sending...';fetch(PP_API+'/api/consultation-request',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:nm,email:em,phone:ph,type:tp,notes:nt})}).then(function(r){if(!r.ok)throw new Error('request-failed');return r.json();}).then(function(){form.innerHTML='<div style="text-align:center;padding:30px 0"><h2 style="color:#0e1b4d;margin:0 0 8px">Request Received!</h2><p style="color:#555;font-size:14px;margin:0 0 4px">Thanks '+nm+', we\'ll reach out within 24 hours to schedule your consultation.</p><p style="color:#888;font-size:13px;margin:12px 0 0">Check your email and phone for confirmation.</p></div>';}).catch(function(){btn.disabled=false;btn.textContent='Request Consultation';alert('We could not send your request. Please try again or use the Contact page.');});};
}
var ppDescCache=null;
var DESCS_URL='https://jsonblob.com/api/jsonBlob/019df526-aee8-70ce-9610-299fe89ad9fd';
window.ppExtraDesc=window.ppExtraDesc||{
'research-starter-kit':'<p><strong>Everything you need for one full peptide reconstitution &mdash; FREE with this kit.</strong></p><ul><li>15 insulin syringes (ultra-fine for daily dosing)</li><li>15 alcohol prep pads (sterile, individually sealed)</li><li>1 large reconstitution needle (for drawing bacteriostatic water)</li><li>FREE reconstitution kit &mdash; included at no extra cost</li></ul><p>Research grade. Third-party tested for purity. Veteran-owned, USA-made.</p><em>For research use only. Not for human consumption.</em>',
'retatrutide-10mg':'<p><strong>Retatrutide 10mg</strong> is a high-interest research listing for researchers comparing modern metabolic peptide documentation. The page is designed to keep the purchase path and the verification path together.</p><ul><li>Review batch documentation on the <a href="/lab-testing">Lab Testing &amp; COAs</a> page before ordering.</li><li>Use the <a href="/post/how-to-read-a-peptide-coa-in-2026-hplc-mass-spec-and-lot-matching">COA guide</a> to check identity, purity, and lot matching.</li><li>Confirm storage and handling details before the vial is moved into a study workflow.</li></ul><p>Research use only. Not for human consumption. Third-party testing and documentation are available for review.</p>',
'retatrutide-20mg':'<p><strong>Retatrutide 20mg</strong> is a higher-quantity research listing for teams that want the same documentation-first checkout flow with a larger vial size. The listing is paired with the same verification resources as the 10mg option.</p><ul><li>Review batch documentation on the <a href="/lab-testing">Lab Testing &amp; COAs</a> page before ordering.</li><li>Use the <a href="/post/how-to-read-a-peptide-coa-in-2026-hplc-mass-spec-and-lot-matching">COA guide</a> to check identity, purity, and lot matching.</li><li>Confirm storage and handling details before the vial is moved into a study workflow.</li></ul><p>Research use only. Not for human consumption. Third-party testing and documentation are available for review.</p>'
};
function loadDescs(){if(ppDescCache)return Promise.resolve(ppDescCache);return fetch(DESCS_URL).then(function(r){return r.json();}).then(function(d){ppDescCache=Object.assign({},d||{},window.ppExtraDesc||{});return ppDescCache;}).catch(function(){ppDescCache=window.ppExtraDesc||{};return ppDescCache;});}
loadDescs();
function initModal(){
if(!isHome)return;
...
if(!prod)return;
document.getElementById('pm-img').src=prod[1][0]==='h'?prod[1]:B+prod[1]+'/v1/fit/w_800,h_800,q_85,enc_auto/'+prod[1];
...
document.getElementById('pm-buy').onclick=function(){if(window.ppCart){ppCart.add(curSlug);window.location.href='/cart-page'+((window.ppAffiliate&&ppAffiliate.applied)?('?promo='+ppAffiliate.applied):'');}};
};
}
function initCartPage(){if(!isCart||!document.body)return;injectPageShell();if(window.ppCartPage)ppCartPage();}
function initBlog(){if(!isBlog||!document.body||document.getElementById('pp-blog'))return;injectPageShell();var posts=[
  ['Research Product Documentation','How to review identity, purity, lot numbers, and supporting laboratory records.'],
  ['Reading a Certificate of Analysis','A practical guide to test methods, reported values, and document verification.'],
  ['Research Storage Fundamentals','General handling and storage considerations for laboratory research materials.'],
  ['Third-Party Testing Explained','What independent testing can establish and what it cannot establish.'],
  ['Building a Research Checklist','A repeatable checklist for documentation, labeling, handling, and recordkeeping.'],
  ['Understanding Product Labels','How to compare product identity, lot information, and approved reference records.']
];var bl=document.createElement('div');bl.id='pp-blog';bl.className='pp-cf';bl.style.maxWidth='900px';bl.innerHTML='<div style="display:grid;gap:14px;margin:18px 0 26px;padding:22px;border:1px solid #dfe5f3;border-radius:16px;background:linear-gradient(180deg,#f8faff,#eef3fb)"><p class="pp-badge" style="margin:0">RESEARCH EDUCATION</p><h1 style="margin:0;font-size:34px;line-height:1.08">Precision Labs Research Education</h1><p style="margin:0;max-width:760px;text-align:left;font-size:15px;line-height:1.7;color:#3f4a67">Documentation-first guides for qualified laboratory researchers. Start with lab-testing standards, COA reading, and product documentation best practices.</p><div style="display:flex;flex-wrap:wrap;gap:10px"><a href="/lab-testing" style="display:inline-block;padding:11px 16px;border-radius:999px;background:#0e1b4d;color:#fff;text-decoration:none;font-weight:700;font-size:13px">Lab Testing &amp; COAs</a><a href="/post/how-to-verify-peptide-purity-coa-testing-guide" style="display:inline-block;padding:11px 16px;border-radius:999px;background:#fff;color:#0e1b4d;text-decoration:none;font-weight:700;font-size:13px;border:1px solid #cbd5ea">COA Guide</a><a href="/product-page/retatrutide-10mg" style="display:inline-block;padding:11px 16px;border-radius:999px;background:#fff;color:#0e1b4d;text-decoration:none;font-weight:700;font-size:13px;border:1px solid #cbd5ea">Retatrutide 10mg</a></div></div><div id="pp-blog-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:16px;margin-top:24px"></div>';var anchor=document.getElementById('pp-hdr');if(anchor&&anchor.parentNode)anchor.parentNode.insertBefore(bl,anchor.nextSibling);else document.body.appendChild(bl);var grid=document.getElementById('pp-blog-grid');posts.forEach(function(p){var a=document.createElement('article');a.style.cssText='background:#f7f8fc;border:1px solid #e1e4ee;border-radius:8px;padding:20px';a.innerHTML='<h2 style="font-size:17px;margin:0 0 8px">'+p[0]+'</h2><p style="text-align:left;margin:0;font-size:13px">'+p[1]+'</p>';grid.appendChild(a);});try{var blocks=document.querySelectorAll('section,div');for(var i=0;i<blocks.length;i++){var txt=(blocks[i].textContent||'').replace(/\s+/g,' ').trim();if(txt==='WELCOME'||txt.indexOf('Welcome visitors to your site with a short, engaging introduction.')!==-1){if(txt.length<1000){blocks[i].style.display='none';blocks[i].setAttribute('aria-hidden','true');}}}}catch(e){}}
...
+'<h2>11. Assumption of Risk</h2><p style="text-align:left"><strong>YOU EXPRESSLY ACKNOWLEDGE AND AGREE THAT YOUR PURCHASE, POSSESSION, HANDLING, STORAGE, AND USE OF ANY PRODUCTS FROM PRECISION LABS IS AT YOUR SOLE AND EXCLUSIVE RISK.</strong> You assume full responsibility for ensuring that all products are used in compliance with applicable laws, regulations, and safety protocols.</p>'
...
var coaCards='';COA_DATA.forEach(function(c){var _fn=(c[1].split('/media/')[1]||'');var _t=_fn?c[1]+'/v1/fit/w_600,h_600,q_88,enc_auto/'+_fn:c[1];var _z=_fn?c[1]+'/v1/fit/w_1400,h_1400,q_90,enc_auto/'+_fn:c[1];coaCards+='<div class="pp-coa-card"><div class="pp-coa-label">'+c[0]+'</div><img src="'+_t+'" alt="COA - '+c[0]+'" loading="lazy" onclick="window.ppCoaZoom(\''+_z+'\',\''+c[0].replace(/'/g,"\\'")+'\')"></div>';});
var ltHTML='<button class="pp-lt-close" onclick="this.parentElement.remove()">&times;</button><div class="pp-lt-inner"><h1 style="font-family:Archivo,sans-serif;font-size:26px;font-weight:700;color:#0e1b4d;margin:0 0 8px;text-align:center">Lab Testing & COAs</h1><p style="display:inline-block;background:#0e1b4d;color:#fff;padding:5px 14px;border-radius:20px;font-size:12px;font-weight:700;letter-spacing:1px;margin:0 auto 14px;text-align:center">QUALITY ASSURANCE</p><p style="color:#555;font-size:14px;text-align:center;margin:0 0 10px;line-height:1.6">Batch-by-batch documentation, HPLC purity, and mass-spectrometry verification for every product.</p><p style="color:#555;font-size:13px;text-align:center;margin:0 0 16px;line-height:1.6">After you review a COA, jump back to <span onclick=\"window.location.href=\'/#pp-shop\'\" style=\"color:#4770db;cursor:pointer;font-weight:700\">best sellers</span> or <span onclick=\"window.location.href=\'/contact\'\" style=\"color:#4770db;cursor:pointer;font-weight:700\">request a specific lot number</span>.</p>'
...
;(function(){
  var PP_REVIEWS=[
    {name:'Alexandra P.',rating:5,product:'Precision Labs order',date:'Jul 2026',badge:'Customer submitted',text:"The ordering process was straightforward, communication was responsive, and the documentation was easy to review. Everything arrived carefully packaged and clearly labeled."},
    {name:'Sarah S.',rating:5,product:'Research Circle membership',date:'Jul 2026',badge:'Customer submitted',text:"The independent third-party testing and batch documentation stood out immediately. The team answered my questions promptly, never rushed the conversation, and made the research information easy to understand."}
  ];
