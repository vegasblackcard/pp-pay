...
;(function(){try{
  ...
  var REVIEW_ENDPOINT='https://reel-boxed-limit-blanket.trycloudflare.com/track';
...
(function(){
  ...
  var SEO_TITLE='Precision Labs | Buy Research Peptides Online | BPC-157, TB-500 & GLP';
  var metas=[
    ['description','Shop Precision Labs research peptides including BPC-157, TB-500, GLP research compounds, NAD+, and more with third-party testing, COA support, discreet US shipping, and secure checkout.'],
    ...
    ['og:description','Shop Precision Labs research peptides including BPC-157, TB-500, GLP research compounds, NAD+, and more with third-party testing, COA support, discreet US shipping, and secure checkout.'],
    ...
    ['twitter:description','Shop Precision Labs research peptides including BPC-157, TB-500, GLP research compounds, NAD+, and more with third-party testing, COA support, discreet US shipping, and secure checkout.'],
...
(function(){
  ...
  function showGate(){
    ...
    document.getElementById('pp-av-enter').onclick=function(){
      ...
      try{var x=new XMLHttpRequest();x.open('POST','https://reel-boxed-limit-blanket.trycloudflare.com/track',true);x.setRequestHeader('Content-Type','application/json');x.send(JSON.stringify({vid:(localStorage.getItem('pp_vid')||''),event:'lead_capture',detail:lead,ts:Date.now(),page:location.pathname}));}catch(e){}
...
/* ============================================================
   ...
   ============================================================ */
(function(){
  var ANALYTICS_URL='https://reel-boxed-limit-blanket.trycloudflare.com';
...
'<div style="display:flex;align-items:center;gap:10px"><svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#4770db" stroke-width="2"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2"/><rect x="9" y="3" width="6" height="4" rx="1"/><path d="M9 14l2 2 4-4"/></svg><div><div style="color:#fff;font-size:14px;font-weight:700;letter-spacing:.5px">COA EVERY BATCH</div><div style="color:#abb8e6;font-size:11px">Documentation Support</div></div></div>'+
...
'<div style="display:flex;align-items:center;gap:10px"><svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#4770db" stroke-width="2"><rect x="1" y="3" width="15" height="13" rx="2"/><path d="M16 8h4l3 3v5a2 2 0 01-2 2h-1"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg><div><div style="color:#fff;font-size:14px;font-weight:700;letter-spacing:.5px">DISCREET US SHIPPING</div><div style="color:#abb8e6;font-size:11px">Plain Packaging</div></div></div>'+
'<div style="display:flex;align-items:center;gap:10px"><svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#4770db" stroke-width="2"><path d="M3 12a9 9 0 101.9-5.5"/><path d="M3 4v6h6"/></svg><div><div style="color:#fff;font-size:14px;font-weight:700;letter-spacing:.5px">RESHIP GUARANTEE</div><div style="color:#abb8e6;font-size:11px">Damaged Order Support</div></div></div>'+
'</div></div>'
...
try{var _pc=(window.ppAffiliate&&ppAffiliate.applied)||'';if(_pc){try{var _em=(document.getElementById('pp-ship-email')||{}).value||'';navigator.sendBeacon('https://soap-meant-greetings-workforce.trycloudflare.com/api/promo-track',JSON.stringify({code:_pc,method:method,amount:amt,email:_em}));}catch(e){}}}catch(e){}
try{
...
var trust='<div class="pp-product-trust" style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:12px 0 14px"><a href="/lab-testing" style="border:1px solid #dfe4f0;border-radius:8px;padding:9px 7px;text-align:center;text-decoration:none;color:#0e1b4d;background:#f8f9fc"><strong style="display:block;font-size:11px">COA LINK</strong><span style="font-size:10px;color:#667">Request batch file</span></a><div style="border:1px solid #dfe4f0;border-radius:8px;padding:9px 7px;text-align:center;color:#0e1b4d;background:#f8f9fc"><strong style="display:block;font-size:11px">99%+ PURITY</strong><span style="font-size:10px;color:#667">HPLC verified</span></div><div style="border:1px solid #dfe4f0;border-radius:8px;padding:9px 7px;text-align:center;color:#0e1b4d;background:#f8f9fc"><strong style="display:block;font-size:11px">3RD-PARTY</strong><span style="font-size:10px;color:#667">Lab tested</span></div></div>';
if(html){descEl.innerHTML=trust+html;discEl.style.display='none';}else{descEl.innerHTML=trust+'<p>Research-grade peptide from Precision Labs with third-party testing, 99%+ purity documentation, and COA support for every batch. This compound is stocked for laboratory research workflows that require clear labeling, careful handling, and reliable US fulfillment. Product documentation is available through the Lab Testing page or by request before purchase. Orders ship in discreet packaging through secure checkout. For research use only. Not for human or animal consumption.</p>';discEl.style.display='block';}
});
...
;(function(){try{var m=location.search.match(/[?&]promo=([A-Za-z0-9]+)/);if(!m)return;var c=m[1].toUpperCase();function go(){try{if(window.ppAffiliate&&ppAffiliate.codes[c]){ppAffiliate.apply(c);if(window.ppCartRefresh)ppCartRefresh();try{var _v=localStorage.getItem('pp_vid')||'';var _src=new URLSearchParams(location.search).get('src')||'';fetch('https://reel-boxed-limit-blanket.trycloudflare.com/api/promo-event',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'promo_visit',code:c,source:_src,referrer:document.referrer,vid:_v,ts:Date.now()})}).catch(function(){});}catch(e){}try{var _sh=document.getElementById('pp-shop');if(_sh)_sh.scrollIntoView({behavior:'smooth'});}catch(e){}try{var _t=document.createElement('div');_t.textContent=(ppAffiliate.codes[c].label||c)+' applied ✓';_t.style.cssText='position:fixed;top:64px;left:50%;transform:translateX(-50%);background:#0e1b4d;color:#fff;padding:12px 22px;border-radius:30px;font:600 14px system-ui,sans-serif;z-index:200000;box-shadow:0 6px 24px rgba(0,0,0,.3)';document.body.appendChild(_t);setTimeout(function(){_t.style.transition='opacity .5s';_t.style.opacity='0';setTimeout(function(){_t.remove();},500);},3500);}catch(e){}}}catch(e){}}var _done=false;function _go2(){if(_done)return;if(window.ppAffiliate&&ppAffiliate.codes[c]){_done=true;go();}}function poll(n){_go2();if(_done||n<=0)return;setTimeout(function(){poll(n-1);},400);}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){poll(30);});else poll(30);}catch(e){}})();
/* v26b-high-intent-collections — crawlable internal collection pages */
;(function(){try{
  var PATH=(location.pathname||'/').replace(/\/+$/,'').toLowerCase()||'/';
  var MAP={
    '/buy-bpc-157':{title:'Buy BPC-157 Research Peptides | Precision Labs',h1:'Buy BPC-157 Research Peptides',desc:'Shop BPC-157 research products from Precision Labs with 99%+ purity standards, third-party testing, COA support, secure checkout, and discreet US shipping.',keys:['bpc-157','wolverine','glow-stack','alcohol-research-stack']},
    '/tb-500-for-sale':{title:'TB-500 for Sale | Research Peptides | Precision Labs',h1:'TB-500 Research Peptides for Sale',desc:'Find TB-500 research peptides and BPC-157/TB-500 stacks with batch documentation, purity badges, third-party testing, and secure Precision Labs checkout.',keys:['tb500','tb-500','wolverine','glow-stack','klow']},
    '/glp-research-peptides':{title:'GLP Research Peptides | Semaglutide, Tirzepatide & Retatrutide',h1:'GLP Research Peptides',desc:'Shop GLP research peptides including Semaglutide, Tirzepatide, Retatrutide, Cagrilintide, and MetaLean+ G3 with COA support and discreet US shipping.',keys:['semaglutide','tirzepatide','retatrutide','cagrilintide','metalean','diabetes-research-stack']}
  };
  var cfg=MAP[PATH];if(!cfg)return;
  function setMeta(n,v,p){var s=p?'meta[property="'+n+'"]':'meta[name="'+n+'"]',m=document.querySelector(s);if(!m){m=document.createElement('meta');m.setAttribute(p?'property':'name',n);document.head.appendChild(m);}m.setAttribute('content',v);}
  function esc(s){return String(s||'').replace(/[&<>"']/g,function(c){return({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'})[c];});}
  function img(p){var B='https://static.wixstatic.com/media/';return p[1]&&p[1][0]==='h'?p[1]:B+p[1]+'/v1/fit/w_420,h_420,q_82,enc_auto/'+p[1];}
  function products(){var A=[].concat(window.ppProducts||[],window.ppExtra||[]),out=[];for(var i=0;i<A.length;i++){var hay=(A[i][0]+' '+A[i][2]).toLowerCase();for(var k=0;k<cfg.keys.length;k++){if(hay.indexOf(cfg.keys[k])>-1){out.push(A[i]);break;}}}return out;}
  function boot(){
    document.title=cfg.title;setMeta('description',cfg.desc);setMeta('og:title',cfg.title,true);setMeta('og:description',cfg.desc,true);setMeta('twitter:title',cfg.title,true);setMeta('twitter:description',cfg.desc,true);
    var c=document.querySelector('link[rel="canonical"]')||document.createElement('link');c.rel='canonical';c.href='https://www.precisionusalabs.com'+PATH;if(!c.parentNode)document.head.appendChild(c);
    var list=products();var old=document.getElementById('pp-collection');if(old)old.remove();
    var st=document.createElement('style');st.id='pp-collection-css';st.textContent='body{background:#eff0f5!important;opacity:1!important}#PAGES_CONTAINER,#SITE_PAGES,#masterPage,#SITE_CONTAINER,#site-root,#SITE_HEADER,#SITE_FOOTER,header,footer,[id^=comp-]{display:none!important}.pp-col{font-family:Questrial,Arial,sans-serif;color:#0e1b4d;max-width:1180px;margin:0 auto;padding:74px 18px 36px}.pp-col h1{font:800 32px Archivo,Arial,sans-serif;margin:0 0 10px;text-align:center}.pp-col p{line-height:1.7;color:#556;max-width:820px;margin:0 auto 18px;text-align:center}.pp-col-trust{display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin:18px 0 24px}.pp-col-trust span{background:#fff;border:1px solid #dfe4f0;border-radius:8px;padding:9px 12px;font-size:12px;font-weight:800}.pp-col-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:18px}.pp-col-card{background:#fff;border-radius:8px;padding:14px;text-align:center;border:1px solid #e4e8f2}.pp-col-card img{width:100%;aspect-ratio:1/1;object-fit:cover}.pp-col-card h2{font:700 15px Archivo,Arial,sans-serif;margin:10px 0 4px}.pp-col-card a{display:block;background:#4770db;color:#fff;text-decoration:none;border-radius:40px;padding:10px 12px;font-size:13px;font-weight:800;margin-top:10px}.pp-col-links{display:flex;justify-content:center;gap:10px;flex-wrap:wrap;margin-top:26px}.pp-col-links a{color:#4770db;font-weight:800}';document.head.appendChild(st);
    var html='<main id="pp-collection" class="pp-col"><h1>'+esc(cfg.h1)+'</h1><p>'+esc(cfg.desc)+' Every listing includes a visible purity badge, third-party-tested seal, and a direct COA request path so researchers can review documentation before ordering.</p><div class="pp-col-trust"><span>Discreet US shipping</span><span>Secure checkout</span><span>COA every batch</span><span>Reship guarantee</span></div><div class="pp-col-grid">';
    for(var i=0;i<list.length;i++){html+='<article class="pp-col-card"><img src="'+img(list[i])+'" alt="'+esc(list[i][0])+'" loading="lazy"><h2>'+esc(list[i][0])+'</h2><div style="font-weight:800">$'+list[i][3]+'</div><div style="font-size:11px;color:#667;margin-top:6px">99%+ purity | third-party tested | COA support</div><a href="/?product='+encodeURIComponent(list[i][2])+'#pp-shop">View Product</a></article>';}
    html+='</div><div class="pp-col-links"><a href="/buy-bpc-157">Buy BPC-157</a><a href="/tb-500-for-sale">TB-500 for sale</a><a href="/glp-research-peptides">GLP research peptides</a><a href="/">All products</a></div></main>';
    document.body.insertAdjacentHTML('afterbegin',html);
    var ld={"@context":"https://schema.org","@type":"CollectionPage","name":cfg.h1,"description":cfg.desc,"url":"https://www.precisionusalabs.com"+PATH,"mainEntity":{"@type":"ItemList","itemListElement":list.map(function(p,i){return{"@type":"ListItem","position":i+1,"item":{"@type":"Product","name":p[0],"url":"https://www.precisionusalabs.com/product-page/"+p[2],"image":img(p),"brand":{"@type":"Brand","name":"Precision Labs"},"offers":{"@type":"Offer","priceCurrency":"USD","price":String(p[3]),"availability":"https://schema.org/InStock"}}};})}};var s=document.createElement('script');s.type='application/ld+json';s.id='pp-collection-ld';s.textContent=JSON.stringify(ld);document.head.appendChild(s);
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();
}catch(e){}})();
